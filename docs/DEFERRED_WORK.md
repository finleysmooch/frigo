...
| OB-23 | **1000-row-cap watch list (adopt `fetchAllRows` if scope grows).** These read large tables but are bounded today (can't exceed 1000), so left unchanged in the 2026-06-16 sweep: `statsService.getBookStats` (per-book ≤~200), `mealPlanService.getMealPlanItems`/`getMealPlanSummary` (per-meal), `annotationService.getAnnotationStats` (per user+recipe), `bookViewService.getAllAuthors` (~314 books), `recipeHistoryService.getFriendsCookingInfo` (friends' posts). Also: the two already-fixed inline loops (`RecipeListScreen.loadRecipes`, `getCookingHistory`) could adopt the shared `lib/utils/fetchAllRows` helper for consistency. | 🔧 | 🟢 | Cap sweep, 2026-06-16. Monitor; no current bug. |
...
| OB-7 | **Neutral link-create primitive (rides OB-6).** CP6b's `recipeDeliveryService` inlines a shape-faithful `user_books` insert rather than importing `createUserBookOwnership` (which lives in `recipeExtraction/bookService` — importing it would break the delivery service's isolation from the extraction copy path). Extract a neutral link-create primitive both the extraction path and `recipeDeliveryService` consume, removing the duplicate insert. Ride the OB-6 `getUserBooks`/`bookService` consolidation. | 🔧 | 🟢 | F&F exception (anchor v0.3.7, 2026-06-10); tech debt, not a blocker. |
