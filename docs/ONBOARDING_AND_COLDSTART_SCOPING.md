...
**Version:** v0.3.11 · **Status:** 🟢 reconciled to the live build model · 2026-06-12
...
- **Library linkage (RESOLVED from code):** both `getUserBooks` variants (recipeExtraction/bookService — the one the library UI / BookSelectionModal calls — and bookViewService) scope a user's library through the `user_books` join; neither uses `books.user_id` or `is_catalog`. So delivery = create a `user_books(user_id, catalog_book_id)` row via a shape-faithful inline user_books insert inside recipeDeliveryService (**F&F exception, ruled 2026-06-10**: the isolation requirement — no imports from the extraction path — took precedence over reusing createUserBookOwnership, which lives in recipeExtraction/bookService; the insert is column-faithful with ownership_claimed=true, proof null — proof is canonical in book_ownership_verifications — and recipe_count=0 trigger-maintained; both code sites carry cross-referencing comments). DEFERRED: extract a neutral link-create primitive both paths consume, riding the getUserBooks/bookService consolidation + deep-copy recipes with `book_id = catalog_book_id` under the user's `user_id`. `is_catalog` stays true (orthogonal to ownership). No leakage (recipes user-scoped, books `user_books`-scoped); no new structural decision needed.
...
| CP-spaces | shared-pantries corrective: check_space_permission (server-truth matrix + self-or-service guard) + recursive space_members policies recreated via definer helper + checkPermission fail-loud | GATED | ✅ shipped + prod-verified (spouse-case harness checks 2–4 PASS; non-member + cross-user denial probes PASS; 2026-06-12 SESSION_LOG) |
...
| Date | Version | Change |
|------|---------|--------|
| 2026-06-12 | 0.3.11 | CP-spaces shipped + prod-verified (check_space_permission server-truth matrix + D-ON-18 self-or-service guard + last-owner-leave guard; recursive space_members policies recreated via definer helper; checkPermission fail-loud). D-ON-16/17 UNGATED — join-pantry branch live. D-ON-18 added. Deferred: members-invite-members product question; definer-helper p_user_id exposure review; get_books_with_counts orphan; UserSearchScreen fire-and-forget counts. |
