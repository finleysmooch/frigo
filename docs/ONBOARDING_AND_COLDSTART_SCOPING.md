...
**Version:** v0.3.5 · **Status:** 🟢 reconciled to the live build model · 2026-06-10
...
- **Copy scope (Tom's decision of record — risk accepted, against oversight rec):** for F&F, copy the **full recipe** — functional fields **plus** the protected categories (`description`, `recipe_notes`, `recipe_photos`, `recipe_media`). The carve-out is **removed** for F&F. **Ratified copy-set (2026-06-10).** COPY: `instruction_sections` (+ steps), `recipe_ingredients`, `recipe_media` (reference copy per §3), `recipe_photos` (copy-if-present; expected empty — zero writers, vestigial), `recipe_source_notes` (canonical source-derived content; note it is third-party UGC from the source site — inside the F&F posture and the counsel gate). EXCLUDE: `recipe_annotations`, `user_recipe_tags`, `user_recipe_preferences`, **`recipe_step_notes`** (user content — `user_id NOT NULL`, carries voice notes, sole writer `cookingService`), `recipe_references` (leak/dangle; config hook if overruled), user-activity tables (`cooking_sessions`, `posts`, `meal_dish_plans`, `needs_recipes`), QA/operational artifacts (`extraction_*`, `recipe_extraction_comparison`/`queue`, `recipe_image_mappings`, `or_pattern_decisions`; `gold_standard_notes` column). **Completeness guard:** the engine CP runs a live FK scan (`pg_constraint` where `confrelid = 'recipes'::regclass`) and STOPS if any child table appears that is not classified here — the allow-list is verified against the live schema, never asserted (the PK schema CSVs were found stale pre-June). **Two reversibility requirements are NON-NEGOTIABLE:** (1) the copy-set is a **parameterized config allow-list** the deep-copy reads — a counsel/GA narrowing (→ metadata model) is a config edit, not an engine rebuild; (2) delivered content is **purge-identifiable as a set** (`parent_recipe_id NOT NULL` + provenance stamps make every CP6b-delivered row + children queryable) — so a narrowing can claw back already-delivered prose/images, not just stop forward delivery. These two make the risk acceptance reversible in practice; without them it's a one-way door.
...
| CP6b | Copy-on-verify delivery + provenance (full-recipe copy per §4.3; recipeDeliveryService) | **gated** | 🟡 provenance migration authored (held — pushes with the engine); engine cleared to author on v0.3.5 landing |
