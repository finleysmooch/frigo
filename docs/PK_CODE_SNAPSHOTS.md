...
| `lib/types/grocery.ts` | 2026-04-22 | Phase 8A-CP1 / 8C-CP1 / 8C-CP1a / 8C-CP2 / 8C-CP2a / 8C-CP3 / 8C-CP4 | HIGH | Grocery list/item/template types; 8A-CP1 added priority_reason / custom_name, ingredient_id nullable; 8C-CP1 added typical_store_section to GroceryListItemWithIngredient.ingredient and now/could_wait/in_cart counts to GroceryListWithCounts; 8C-CP1a added store_name to GroceryList and storeName? to Create/UpdateGroceryListParams (resolves D8-35); 8C-CP2 added CrossListIngredientPresence interface; 8C-CP2a added GroceryListItemRecipe interface, extended GroceryListItemWithIngredient with optional recipes? field, fixed added_from enum bug ('template' → 'regular') in both GroceryListItem.added_from and AddedFrom type; 8C-CP3 added view_mode field to GroceryList and viewMode? to UpdateGroceryListParams; 8C-CP4 added source_staple_id to GroceryListItem and extended added_from + AddedFrom enums with 'staple' |
...
| `lib/groceryListsService.ts` | 2026-04-22 | Phase 8C-CP1 / 8C-CP1a / 8C-CP2 / 8C-CP2a / 8C-CP3 / 8C-CP4 | HIGH | 8C-CP1 deleted inline GroceryList/GroceryListItem types in favor of canonical lib/types/grocery; widened getItemsForList SELECT (typical_store_section etc.); widened updateListItem signature (priority/priority_reason/brand/size/custom_name); added getUserGroceryListsWithCounts (single batched query with tier counts). 8C-CP1a renamed local CreateGroceryListParams.store_name → storeName (DB column stays snake_case). 8C-CP2 added getOtherListsContainingIngredient and deleteItemsByIngredientFromLists for cross-list checkoff prompt. 8C-CP2a widened AddItemToListParams with recipeId/recipeQuantityAmount/recipeQuantityUnit; rewrote addItemToList to write junction rows on both insert and merge paths; added private upsertItemRecipeAttribution helper (additive-on-conflict via read-then-write; PostgREST doesn't expose ON CONFLICT DO UPDATE additive math); added getRecipesForItem and getItemsWithRecipes (single batched junction query, no N+1). 8C-CP3 added getGroceryList(listId) and updateGroceryList(listId, params) — camelCase params mapped to snake_case columns. 8C-CP4 widened updateListItem with source_staple_id; folded P8-19 fix into addIngredientsToDefaultList (forwards recipeId/recipeQuantityAmount/recipeQuantityUnit so junction rows write on the recipe→default-list path). ⚠️ Currently at lib/ root; should relocate to lib/services/ per FRIGO_ARCHITECTURE — tracked as T4 in DEFERRED_WORK. |
...
| `lib/pantryStaplesService.ts` | 2026-04-23 | Phase 8B-CP1 / 8B-CP3 / 8B-CP3a / 8C-CP4 | HIGH | Staples CRUD + state cycling; 8B-CP3 added `searchIngredientsForStapleAdd`; 8B-CP3a added case-insensitive + cross-boundary dedup via `throwIfDisplayNameTaken` helper. 8C-CP4 added `routeStapleToGroceryList(stapleId)` (resolves acting user via auth.getUser, finds primary list = user's most-recently-updated active list or auto-creates 'Groceries', three-stage dedup: source_staple_id → ingredient_id/custom_name → insert with added_from='staple'); both `cycleStapleState` and `setStapleState` invoke routing on transitions to 'out' with try/catch soft-fail; imports createGroceryList + updateListItem from groceryListsService. ⚠️ Currently at lib/ root; should relocate to lib/services/ per FRIGO_ARCHITECTURE — tracked as T4 in DEFERRED_WORK. |
...
| `screens/GroceryListDetailScreen.tsx` | 2026-04-22 | Phase 8C-CP1 / 8C-CP2 / 8C-CP3 / 8C-CP4 | HIGH | 8C-CP1 restructured around 3 tiers (Now / Could wait / In cart) with aisle sub-headers; long-press → tier-move Alert picker; renders new presentational GroceryListItem component. 8C-CP2 added cross-list checkoff prompt state + handleToggleItem cross-list query on check-on + Keep/Remove handlers + CrossListPrompt sibling render. 8C-CP3 added per-list viewMode + activeFilter + disambiguationState; switched loadItems to getItemsWithRecipes; hydrateViewMode on mount; ViewModeToggle SVG icon in header; For: strip + FilterChip (mutually exclusive); RecipeDisambiguationSheet Modal; tierGroups memo filters by activeFilter. 8C-CP4 added reverse-direction restore in handleToggleItem (on check-on of a row with source_staple_id, calls setStapleState('good') with try/catch soft-fail; ordered before existing CP2 cross-list prompt; does NOT fire on un-check or delete) |
...
| `components/GroceryListItem.tsx` | 2026-04-22 | Phase 8C-CP1 / 8C-CP3 | HIGH | 8C-CP1 wholesale rewrite: presentational tier-aware row, props-driven, no service imports, long-press triggers onMoveTier. 8C-CP3 widened props with viewMode + onRecipePillTap; replaced priority_reason subtitle with inline staple pill (Compact + Detailed); added tappable recipe pill in Detailed mode (single-name or "{N} recipes" form); pill order: staple first, recipe second |
