...
| `components/ExpandedRegularsSheet.tsx` | 2026-05-19 | Phase 8R-CP5b / CP6b | HIGH | NEW (8R-CP5b, ~590 lines): Out (pre-selected per Q20)/Low/In stock multi-select with inline supply_id-based dedup (createNeed lacks dedup pre-CP6a; CP6a hoisted to service). CP6b: footer "+ Add new supply" rewired from Alert stub to SupplyCreateSheet open with inline refetch. 2026-06-04: "Regulars" header + alert title → "Staples" (Regular→Staple rename). |
