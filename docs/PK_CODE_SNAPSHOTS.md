...
| `lib/groceryListsService.ts` | 2026-04-22 | Phase 8C-CP1 / 8C-CP1a / 8C-CP2 / 8C-CP2a | HIGH | 8C-CP1 deleted inline GroceryList/GroceryListItem types in favor of canonical lib/types/grocery; widened getItemsForList SELECT (typical_store_section etc.); widened updateListItem signature (priority/priority_reason/brand/size/custom_name); added getUserGroceryListsWithCounts (single batched query with tier counts). 8C-CP1a renamed local CreateGroceryListParams.store_name → storeName (DB column stays snake_case). 8C-CP2 added getOtherListsContainingIngredient and deleteItemsByIngredientFromLists for cross-list checkoff prompt. 8C-CP2a widened AddItemToListParams with recipeId/recipeQuantityAmount/recipeQuantityUnit; rewrote addItemToList to write junction rows on both insert and merge paths; added private upsertItemRecipeAttribution helper (additive-on-conflict via read-then-write; PostgREST doesn't expose ON CONFLICT DO UPDATE additive math); added getRecipesForItem and getItemsWithRecipes (single batched junction query, no N+1). ⚠️ Currently at lib/ root; should relocate to lib/services/ per FRIGO_ARCHITECTURE — tracked as T4 in DEFERRED_WORK. |
